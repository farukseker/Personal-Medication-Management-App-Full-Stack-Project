<template>
  <nav class="tabs tabs-lift tabs-md tabs-top">
    ...
    <button @click="go('/tab/counters')" class="tab hover:text-green-500 flex gap-2" :class="isPathEqual('/tab/counters') ? 'tab-active': ''">
      <font-awesome :icon="faPlus" /> {{ $t('index.counter') }}
    </button>
    <button @click="go('/tab/water_tracking')" class="tab hover:text-green-500 flex gap-2" :class="isPathEqual('/tab/water_tracking') ? 'tab-active': ''">
...
<script setup>
import { faPills, faPlus, faGlassWater, faWeightScale } from '@fortawesome/free-solid-svg-icons'
